    ...
    return (
        ...
        >
            ...
            <div
                ...
                className={`flex justify-evenly items-center bg-(--bg-activity-bar) hover:bg-(--accent-color) transition-colors ${isRow
                    ? 'w-1.5 h-full cursor-col-resize user-select-none'
                    : 'w-full h-1.5 cursor-row-resize flex flex-col user-select-none'
                    }`}
            >
                <div
                    className={`bg-(--text-secondary) opacity-50 w-[min(30%,20px)] h-[min(30%,20px)] rounded-full ${isRow ? 'w-0.5 h-8' : 'h-0.5 w-8'
                        }`}
                ></div>
            </div>
