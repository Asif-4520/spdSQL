...
const Statusbar: React.FC<Props> = ({ status, message }) => {
    ...
    return (
        <div className='h-6 w-full bg-(--bg-status-bar) text-white text-[10px] sm:text-xs flex items-center px-1.5 sm:px-2 select-none justify-between border-t border-(--border-color) overflow-hidden'>
            <div className='flex items-center gap-2 sm:gap-3 flex-1 overflow-hidden mr-2 sm:mr-4'>
                ...
                {message && (
                    <button className='truncate max-w-[150px] sm:max-w-125 hover:bg-white/10 px-1 sm:px-1.5 py-0.5 rounded transition-colors text-left flex items-center gap-2 group'>
                        {message}
                    </button>
                )}
            </div>
            ...
            <div className='flex items-center gap-2 sm:gap-4 shrink-0'>
                ...
                <div className='flex items-center gap-2 sm:gap-3'>
                    <span
                        className='cursor-pointer hover:bg-white/10 px-1 rounded'
                        title='Language mode'
                    >
                        SQLio
                    </span>
