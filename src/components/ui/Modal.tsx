...
const Modal: React.FC<Props> = ({
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in p-3 sm:p-2">
            <div
                ref={ref}
                className={`bg-(--bg-panel) border border-(--border-color) shadow-2xl rounded-2xl w-full ${maxWidth} overflow-hidden animate-scale-in max-h-[88vh] flex flex-col`}
            >
                ...
                <div className="p-3 sm:p-5 text-(--text-primary) overflow-y-auto overflow-x-hidden flex-1 min-h-0">
                    {children}
                </div>
            </div>
        </div>
