...
import type { QueryResponse } from '../lib/DB/db.core';
...
export function useQueryRunner() {
    ...
    const resolveQueryText = useCallback(
        (queryOverride?: string) => {
            if (queryOverride && queryOverride.trim()) return queryOverride;
            if (typeof ctx.editorGetValue === 'function') return ctx.editorGetValue();
            return ctx.Query;
        },
        [ctx]
    );
        ...
        async (options?: RunQueryOptions): Promise<RunQueryResult> => {
            ...
            try {
                ...
                if (error) {
                    ...
                    return { success: false, data: null, error, timeMs: duration };
                }
                ...
                return { success: false, data: null, error: message, timeMs: duration };
            }
        },
