...
const detectStatementType = (sql: string): StatementType => {
    ...
    if (s.startsWith('EXPLAIN')) return 'EXPLAIN';
    if (s.startsWith('DESCRIBE') || s.startsWith('DESC')) return 'DESCRIBE';
    if (s.startsWith('SHOW')) return 'SHOW';
    if (s.startsWith('COPY')) return 'COPY';
    ...
    try {
        ...
        if (isDMLStatement(statementType)) {
            ...
            try {
                ...
                // Ignore errors in parsing affected rows
            }
