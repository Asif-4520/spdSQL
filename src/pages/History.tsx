        ...
        <div className='p-6 h-full w-full overflow-auto bg-(--bg-panel) text-(--text-primary)'>
            ...
            {History.length === 0 ? (
                <div className='flex flex-col items-center justify-center h-64 text-(--text-secondary)'>
                    ...
                    <p className='text-sm opacity-70'>Run a query to see it here</p>
                </div>
            ) : (
                        ...
                        >
                            <div className='flex items-start justify-between gap-4'>
                                <div className='flex-1 min-w-0'>
                                    <pre className='text-sm font-mono whitespace-pre-wrap break-words bg-(--bg-panel) p-3 rounded-xl border border-(--border-color) max-h-32 overflow-auto'>
                                        {item.query}
                                    </pre>
                                </div>
                                <button
                            ...
                            <div className='flex items-center gap-4 mt-3 text-xs'>
                                <span
                                    className={`flex items-center gap-1.5 ${item.status === 'success' ? 'text-green-400' : 'text-red-400'
                                        }`}
                                >
                                    {item.status === 'success' ? (
                                        <CheckCircle size={14} />
                                ...
                                <span className='text-(--text-secondary)'>{formatTime(item.timestamp)}</span>
